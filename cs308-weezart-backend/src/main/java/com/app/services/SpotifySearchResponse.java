...
//	}
	...
	public List<Album> albumJsonParser(String albumJson, String token)
		...
		for (JsonNode item : itemsNode) {
			...
			for (JsonNode artist : artistsNode) {
				if (artist.get("name") != null) {
					artistsName.add(artist.get("name").asText());
				}
				if (artist.get("id") != null) {
					artistsId.add(artist.get("id").asText());
				}
			}
			...
			Album album = new Album(id, name, imageUrl, releaseDate, numberOfTracks, artistsName, artistsId, songsName,
					songsId);
			albumList.add(album);
		}
		return albumList;
	...
	public Map<String, String> songFromAlbumParser(String songFromAlbumJson)
			throws JsonMappingException, JsonProcessingException {
